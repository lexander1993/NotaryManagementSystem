Public Class frmTraPaymentOutDet
#Region "Property"

    Private frmParent As frmTraPaymentOut
    Private frmParentNotification As frmSysNotificationPayment
    Private bolNew, bolSave As Boolean
    Private intPaymentOutID As Integer = 0
    ...
    Public Property pubPaymentOutID() As Integer
        Get
            Return intPaymentOutID
        End Get
        Set(ByVal Value As Integer)
            intPaymentOutID = Value
        End Set
    End Property
...
#Region "Function"
    ...
    Private Sub prvFillForm()
        Try
            If Not bolNew Then
                clsData = DL.PaymentOut.GetDetail(intPaymentOutID)

                txtPaymentOutID.Text = intPaymentOutID
                txtPaymentInID.Text = clsData.PInID
    ...
    Private Function ValidateData() As Boolean
        If Not (IsNumeric(txtPaymentInID.Text) = True Or txtPaymentInID.Text = "") Then
            UI.usForm.frmMessageBox("Payment In ID harus numerik !")
            txtPaymentInID.Focus()
            Return False
        End If

        If txtPaymentInID.Text <> "" Then
            If txtPaymentInID.Text <> 0 Then
                If DL.PaymentOut.CheckPIn(txtPaymentInID.Text) = False Then
                    UI.usForm.frmMessageBox("Payment In ID :" & txtPaymentInID.Text & " tidak terdaftar pada Payment In ! ")
                    Return False
                End If
            End If
        End If
    ...
    Private Sub prvSave()
        ...
        clsData.POutID = IIf(bolNew = True, 0, intPaymentOutID)
        clsData.PInID = IIf(txtPaymentInID.Text = "", 0, txtPaymentInID.Text)
        ...
        Try
            ...
            DL.PaymentOut.SaveData(bolNew, clsData)
            bolSave = True
            If bolNew Then
                UI.usForm.frmMessageBox("Save Success Payment Out !")
            Else
                UI.usForm.frmMessageBox("Update Success Payment Out !")
            End If
            Try
                frmParent.pubRefresh(txtNoAkta.Text)
                Me.Close()
            Catch ex As Exception
                frmParentNotification.pubRefresh(intPaymentOutID)
                Me.Close()
            End Try
        Catch ex As Exception
            bolSave = False
...
#Region "Form Handle"
    ...
    Private Sub btnNoAkta_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNoAkta.Click
        Dim frmDetail As New frmTraPaymentIn
        With frmDetail
            .pubIsLookUp = True
            .StartPosition = FormStartPosition.CenterScreen
            .ShowDialog()
            If .pubIsLookUpGet Then
                txtPaymentInID.Text = .pubLUDataRow("PInID")
